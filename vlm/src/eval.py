...
def evaluate_from_predictions(config):
    ...
    if len(eval_preds) != len(test_ds):
        eval_preds['new_index'] = eval_preds.index // config["text_per_image"]
        eval_preds = eval_preds.groupby([
            "new_index",
            "prediction_text",
            "filename",
            "prompt"
        ]).agg({
            "label_text": lambda x: x.tolist()
        }).reset_index().sort_values(by="new_index")
...
if __name__ == "__main__":
    ...
    with open('config/config_evaluation.yml', 'r') as file:
