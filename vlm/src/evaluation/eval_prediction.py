...
def compute_all_metrics(predictions, dataset, text_column):
    ...
    try:
        ...
        testing_cider = compute_cider_scores(
            [re.sub(r"\\.", "", s.encode('unicode_escape').decode()) for s in predictions[:10]],
            [[re.sub(r"\\.", "", s.encode('unicode_escape').decode()) for s in label] for label in labels[:10]],
            metrics["cider"]
        )
    except:
